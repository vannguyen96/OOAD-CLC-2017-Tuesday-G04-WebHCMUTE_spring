...
b<d;
b++){var e=f[b];
if(e.getAttribute("re_target")==null){e.setAttribute("re_target",e.getAttribute("target"));
...
if(c!=null){if(c=="null"){f.removeAttribute("target");
}else{f.setAttribute("target",c);
...
}else{if(h){h.open("ImageManager.html",g,d);
...
this.ApplyClass=function(p,e,d){var n=true;
var f=d.get_value();
var k=e.get_contentWindow();
var j=e.get_document();
var a=new Telerik.Web.UI.Editor.GenericCommand(e.getLocalizedString(p)+' ["'+f+'"]',e.get_contentWindow(),e);
var h=e.getSelectedElement();
var o=(!f&&!e.getSelection().getHtml())||(h&&h.tagName=="UL");
var m=(o)?h:Telerik.Web.UI.Editor.Utils.getFullySelectedElement(k);
if(m){Telerik.Web.UI.Editor.Utils.setElementClassName(m,f);
n=false;
}else{if(!f){j.execCommand("RemoveFormat",false,null);
n=false;
}}if(n){var b=Telerik.Web.UI.Editor.CommandList._markEditorSelection(e);
var l=b.markedElements;
if(l.length>0){for(var g=0;
g<l.length;
g++){var c=l[g];
if(c.className&&c.className!="Apple-style-span"){continue;
}else{c.className=f;
}}if(!$telerik.isIE){Telerik.Web.UI.Editor.Utils.addElementsToSelection(e.get_contentWindow(),l);
...
},getHtmlContent:function(e){var d=e;
var a;
var f=d.getElementsByTagName("body");
var g=f.length;
for(a=0;
a<g;
a++){var k=f[a];
this.convertAttribute(k,"bgColor","backgroundColor");
this.convertAttribute(k,"background","backgroundImage");
}f=d.getElementsByTagName("p");
g=f.length;
for(a=0;
a<g;
a++){this.convertAttribute(f[a],"align","textAlign");
}for(var b=1;
b<7;
b++){var c=d.getElementsByTagName("H"+b);
for(a=0,l=c.length;
a<l;
a++){this.convertAttribute(c[a],"align","textAlign");
}}f=d.getElementsByTagName("div");
g=f.length;
for(a=0;
a<g;
a++){this.convertAttribute(f[a],"align","textAlign");
}f=d.getElementsByTagName("table");
g=f.length;
for(a=0;
a<g;
a++){var k=f[a];
...
this.convertAttribute(k,"bgColor","backgroundColor");
this.convertAttribute(k,"background","backgroundImage");
this.convertAttribute(k,"borderColor","borderColor");
this.convertAttribute(k,"width","width");
this.convertAttribute(k,"height","height");
this.convertAttribute(k,"noWrap","whiteSpace");
}f=d.getElementsByTagName("img");
g=f.length;
for(a=0;
a<g;
a++){var k=f[a];
this.convertImgAlignAttribute(k);
var h=new Array("marginLeft","marginRight");
this.convertAttribute(k,"hspace",h);
h=new Array("marginTop","marginBottom");
this.convertAttribute(k,"vspace",h);
this.convertBorderAttribute(k);
}f=d.getElementsByTagName("ul");
g=f.length;
for(a=0;
a<g;
a++){this.convertTypeAttribute(f[a]);
}f=d.getElementsByTagName("ol");
g=f.length;
for(a=0;
a<g;
a++){this.convertTypeAttribute(f[a]);
}return d;
},getDesignContent:function(e){var d=e;
var a;
var f=d.getElementsByTagName("p");
var g=f.length;
for(a=0;
a<g;
a++){this.convertCSSProperty(f[a],"align","textAlign");
}for(var b=1;
b<7;
b++){var c=d.getElementsByTagName("H"+b);
for(a=0,l=c.length;
a<l;
a++){this.convertCSSProperty(c[a],"align","textAlign");
}}f=d.getElementsByTagName("div");
g=f.length;
for(a=0;
a<g;
...
break;
default:if(!($telerik.isIE9&&d=="br")&&(b.hasChildNodes()||(true==b.canHaveChildren||(b.canHaveChildren==null&&this._canHaveChildren(b))))){a.append(">");
var n=b.childNodes;
k=n.length;
for(var h=0;
h<k;
...
if(d.tagName=="HTML"){var c=(typeof(d.document)!="undefined")?d.document:d.ownerDocument;
...
Telerik.Web.UI.Editor.PdfExportFilter.prototype={getHtmlContent:function(h){var m=h;
var k=m.ownerDocument;
var b=m.getElementsByTagName("TABLE");
for(var e=0,d=b.length;
e<d;
e++){var a=b[e];
var p=a.getAttribute("width");
if(p){a.removeAttribute("width");
a.style.width=p;
}if(!a.style.width){a.style.width="0px";
}var o=a.getElementsByTagName("colgroup");
if(o.length==0){var c=k.createElement("colgroup");
var n=this._getTableColCount(a);
for(var f=0;
f<n;
f++){var g=k.createElement("col");
c.appendChild(g);
...
if(h!=Telerik.Web.UI.EditorContentAreaMode.Div){if(i.body.childNodes.length!=1){i.body.setActive();
}}var f=i.selection;
var e=f.createRange();
...
window.setTimeout(function(){if($telerik.isIE){var j=c.get_contentArea();
if(!j){return;
...
if(this._resizeSelection){var c=(this._resizeSelection.commonParentElement)?this._resizeSelection.commonParentElement():this._resizeSelection.parentElement();
...
h=h.replace(/^[\s\S]*<html[\s\S]+?<body[^>]*>([\s\S]*?)<\/body>[\s\S]*$/gi,"$1");
...
}}else{if(null!=j||true==n){var f="";
if(this.isOpera){f=" style='height:100%'";
}if(!j){j="<head><style></style></head><body"+f+">"+h+"</body>";
...
},_fixBrokenParagraphs:function(h){if($telerik.isIE){var d=this.get_contentArea();
...
}}},pasteHtml:function(j,q,h,f,p){if(!this.get_editable()){return;
}var e=new a.EditorCommandEventArgs(q,null,j);
if(false==this._executeCommandEvent("pasteHtml",q,e)){return;
}if(j!=e.get_value()){j=e.get_value();
}var k=this.getLocalizedString(q);
var c=this.get_mode();
if(a.EditModes.Design==c){this.setFocus();
this.executeCommand(new a.Editor.PasteHtmlCommand(k,this.get_contentWindow(),j,h,this),null,p);
...
d.text=j;
}else{if(g.setSelectionRange){var n=g.selectionStart;
var l=g.selectionEnd;
var m=g.value.substring(n,l);
var o=j;
g.value=g.value.substring(0,n)+o+g.value.substring(l);
g.setSelectionRange(n+o.length,n+o.length);
this.setFocus();
...
var k={};
Array.add(e._onSubmitStatements,function(){d.saveContent(null,k);
return true;
});
...
this._url="Telerik.Web.UI.SpellCheckHandler.html";
